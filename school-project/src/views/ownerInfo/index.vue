    ...
    >
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <span style="color:#43CD80">+{{ scope.row.sum }}</span>
        </template>
      </el-table-column>
      <el-table-column
          align="center"
...
export default {
  ...
  methods: {
    ...
    fuQuery(){
      if(this.content!=""){
        get("/income/findByUserName?userName="+this.content).then((res)=>{
    ...
    //删除
    handleToDelete(id) {
      Del('/income/delete?id='+id).then((res)=>{
        this.$notify({
          title: "删除成功",
    ...
    handleToDel(){
      var param = this.ids+ ""
      Del("/income/deleteBatch?ids="+param).then((res)=>{
        this.$notify({
          title: "批量删除成功",
          message: res.data.data,
          type: "success",
